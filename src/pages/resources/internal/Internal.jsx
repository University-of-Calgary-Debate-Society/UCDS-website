...
export default function Internal() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        // Add drop shadow glow when cursor is near - Red Tinted
        if (this.opacity > this.baseOpacity + 0.05) {
          ctx.shadowBlur = 15;
          ctx.shadowColor = 'rgba(239, 68, 68, 0.9)'; // Red shadow
          ctx.fillStyle = `rgba(254, 202, 202, ${this.opacity})`; // Light pink/red glowing particle
        } else {
          ctx.shadowBlur = 0;
          ctx.fillStyle = `rgba(239, 68, 68, ${this.opacity})`; // Red particle
        }
    ...
    const animate = () => {
      ...
      // 2. Draw Network Lines between close nodes - Red Tinted
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(239, 68, 68, ${opacity})`; // Red lines
      ...
      // 3. Draw clean connection line directly to mouse pointer - Red Tinted
      if (mouse.active) {
        particles.forEach(p => {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(254, 202, 202, ${opacity})`; // Light pink/red line to cursor
            ctx.lineWidth = 1.2;
            ctx.stroke();
          }
        });
  ...
  return (
    <main className="resources-page-container">
      {/* Animated interactive neural network background (Red Tinted) */}
