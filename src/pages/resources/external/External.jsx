...
export default function External() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        // Add drop shadow glow when cursor is near - Blue Tinted
        if (this.opacity > this.baseOpacity + 0.05) {
          ctx.shadowBlur = 15;
          ctx.shadowColor = 'rgba(59, 130, 246, 0.9)'; // Blue shadow
          ctx.fillStyle = `rgba(191, 219, 254, ${this.opacity})`; // Light blue glowing particle
        } else {
          ctx.shadowBlur = 0;
          ctx.fillStyle = `rgba(59, 130, 246, ${this.opacity})`; // Blue particle
        }
    ...
    const animate = () => {
      ...
      // 2. Draw Network Lines between close nodes - Blue Tinted
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(59, 130, 246, ${opacity})`; // Blue lines
      ...
      // 3. Draw clean connection line directly to mouse pointer - Blue Tinted
      if (mouse.active) {
        particles.forEach(p => {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(191, 219, 254, ${opacity})`; // Light blue line to cursor
            ctx.lineWidth = 1.2;
            ctx.stroke();
          }
        });
  ...
  return (
    <main className="resources-page-container">
      {/* Animated interactive neural network background (Blue Tinted) */}
