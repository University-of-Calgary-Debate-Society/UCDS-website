...
export default function Practice() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        // Add drop shadow glow when cursor is near - Yellow Tinted
        if (this.opacity > this.baseOpacity + 0.05) {
          ctx.shadowBlur = 15;
          ctx.shadowColor = 'rgba(234, 179, 8, 0.9)'; // Yellow shadow
          ctx.fillStyle = `rgba(254, 240, 138, ${this.opacity})`; // Light yellow glowing particle
        } else {
          ctx.shadowBlur = 0;
          ctx.fillStyle = `rgba(234, 179, 8, ${this.opacity})`; // Yellow particle
        }
    ...
    const animate = () => {
      ...
      // 2. Draw Network Lines between close nodes - Yellow Tinted
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(234, 179, 8, ${opacity})`; // Yellow lines
      ...
      // 3. Draw clean connection line directly to mouse pointer - Yellow Tinted
      if (mouse.active) {
        particles.forEach(p => {
          ...
          if (dist < limit) {
            ...
            ctx.strokeStyle = `rgba(254, 240, 138, ${opacity})`; // Light yellow line to cursor
            ctx.lineWidth = 1.2;
            ctx.stroke();
          }
        });
  ...
  return (
    <main className="resources-page-container">
      {/* Animated interactive neural network background (Yellow Tinted) */}
