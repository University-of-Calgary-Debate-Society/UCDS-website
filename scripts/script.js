...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (modal) {
    ...
    const nSubmitBtn = document.getElementById('newsletterSubmitBtn');
    const nMsgEl = document.getElementById('newsletterMessage');

    if (nForm) {
      nForm.addEventListener('submit', (e) => {
        e.preventDefault();
        nSubmitBtn.disabled = true;
        nSubmitBtn.innerText = "Submitting...";
        
        nMsgEl.style.display = 'none';
        nMsgEl.innerText = '';
        ...
        .then(() => {
          ...
          nMsgEl.style.display = 'block';
          nMsgEl.style.color = '#16a34a';
          nMsgEl.innerText = "Thank you for subscribing! Check your inbox for updates.";
          nForm.reset();
          
          setTimeout(() => {
            closeModal();
            // Reset button text
            nSubmitBtn.innerText = "Subscribe Now";
            nSubmitBtn.disabled = false;
            nMsgEl.style.display = 'none';
          }, 3000);
        })
        .catch(err => {
          ...
          nSubmitBtn.innerText = "Subscribe Now";
          nMsgEl.style.display = 'block';
          nMsgEl.style.color = '#ef4444';
          nMsgEl.innerText = "There was an error subscribing. Please try again.";
        });
      });
    }
  }
  ...
  // Team Carousel Scroll Indicators
  const teamCarousel = document.querySelector('.team-carousel');
  const dots = document.querySelectorAll('.carousel-indicators .dot');

  if (teamCarousel && dots.length > 0) {
    teamCarousel.addEventListener('scroll', () => {
      const scrollLeft = teamCarousel.scrollLeft;
      const itemWidth = teamCarousel.clientWidth;
    ...
    dots.forEach((dot, index) => {
      dot.addEventListener('click', () => {
        const itemWidth = teamCarousel.clientWidth;
        teamCarousel.scrollTo({ left: itemWidth * index, behavior: 'smooth' });
      });
    });
  }
